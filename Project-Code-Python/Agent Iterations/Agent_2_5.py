                ...
                if difference in ag_pairs:
                    ...
                    if len(B.objects) + len(D.objects) == len(E.objects):
                        ...
                        for object in G.objects:
                            ...
                            #check to see where it was in relation to the other known objects
                            #for prev_object in A.objects:
                                #make sure the object you're looking at did not get deleted
                                #if differences_ac[prev_object] != 'deleted' and differences_ag[prev_object] != 'deleted':
                                    #look at the alignments of the new objects and align them with the new objects



                            #add it in to the alignment attributes of other objects
            ...
            if attribute_name in B_dict:
                ...
                if attribute_name == "left-of":
                    for i in range(len(A_dict[attribute_name])):
                        diff["left-of_{0}".format(i)] = "deleted"
                elif attribute_name == "above":
                    for i in range(len(A_dict[attribute_name])):
                        diff["left-of_{0}".format(i)] = "deleted"
                else:
                ...
                if attribute_name in A_dict:
                    ...
                    if attribute_name == "left-of":
                        for i in range(len(B_dict[attribute_name])):
                            diff["left-of_{0}".format(i)] = "added"
                    elif attribute_name == "above":
                        for i in range(len(B_dict[attribute_name])):
                            diff["left-of_{0}".format(i)] = "added"
                    else:
