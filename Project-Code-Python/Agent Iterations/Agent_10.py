    ...
    def Solve(self,problem):
        ...
        for i in range(len(self.alignments)):
            for object in differences_ab:
                if ab_pairs[object] != "added" and ab_pairs[object] != "deleted":
                    if self.alignments[i] in all_attributes[object] and self.alignments[i] in all_attributes[ab_pairs[object]]:
                        if all_attributes[object][self.alignments[i]] in ab_pairs:
                            if ab_pairs[all_attributes[object][self.alignments[i]]] == all_attributes[ab_pairs[object]][self.alignments[i]]:
        ...
        for i in range(len(self.alignments)):
            for object in differences_ac:
                if ac_pairs[object] != "added" and ac_pairs[object] != "deleted":
                    if self.alignments[i] in all_attributes[object] and self.alignments[i] in all_attributes[ac_pairs[object]]:
                        if all_attributes[object][self.alignments[i]] in ac_pairs:
                            if ac_pairs[all_attributes[object][self.alignments[i]]] == all_attributes[ac_pairs[object]][self.alignments[i]]:
                ...
                if ac_pairs[difference] != "deleted":
                    ...
                    if difference in ab_pairs:
                        if ab_pairs[difference] == "added":
                            ...
                            #check to see what attributes are the same from A, apply them to D from C
                            for attribute_name, attribute_val in all_attributes[ab_pairs[difference]].items():
                                if attribute_val == all_attributes[difference][attribute_name]:
                                    D_C[difference][attribute_name] = all_attributes[ac_pairs[difference]][attribute_name]
                        ...
                        #check to see if the object was deleted from A -> B
                            ...
                            for attribute_name, attribute in differences_ab[difference].items():
                                ...
                                if attribute_name == 'fill':
                                    ...
                                    if 'size' in all_attributes[b_object] and 'size' in all_attributes[difference] and 'size' in all_attributes[c_object]:
